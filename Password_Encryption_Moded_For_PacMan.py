...
def main(file_input):
    Character_Reference_List = build_char_database()
    Output = []
    # operation_choice = ''
    ...
    Input_Password_Vals, input_key = get_vals_from_password('test', int(len(Character_Reference_List)/2-1))
    if input_key == 0:
        input_key = 1
    decrypted_message = decrypt(message_list_generator(file_input), Input_Password_Vals, input_key, Character_Reference_List)
